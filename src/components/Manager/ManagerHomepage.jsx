import axios from 'axios'
import React, { useEffect, useState } from 'react'
import {Card, Container } from 'react-bootstrap'
import Sidebar from './SidebarManager'


function ManagerHomepage() {
  
  const [data, setdata] = useState([])
  const [datas,setdatas] = useState({})

  useEffect(() => {
    axios.get(`http://localhost:8085/Admin/gettoken/${window.localStorage.getItem('jwt')}`)
      .then(response => {
        axios.get(`http://localhost:8082/adminManager/get/${response.data}`)
          .then(responses => {
            setdatas(responses.data)
          }, [])
        
      })
  }, []);


  useEffect(()=>{
    axios.get(`http://localhost:8085/Admin/gettoken/${window.localStorage.getItem('jwt')}`)
    .then(response => {
      axios.get(`http://localhost:8087/ManagerCheck/get/${response.data}`)
         .then(responses => {
            setdata(responses.data);
         })
         .catch(error =>{
            console.log(error);
         });
        })
        },[]);





  return (
  // <AutoLogout>
    <div>
      <Sidebar/>
      <Container>

      <div className='catlog container'>

<div className='row'>

      <div className='col-md-3 prod'>
        <div className='catlogclass'>
          <center>
          <Card style={{ width: '25rem',marginTop:"10px",height:"380px",position:"relative",left: "350px" }} >
            <center>
          <Card.Img  variant="top" style={{height:"150px",width:"180px"}} src="data:image/png;base64," />
          </center>
            <Card.Body>
              
              <Card.Title>{datas.lastname + ' ' +datas.firstname}</Card.Title>
              <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Email Address: {datas.manageremail}</Card.Body>
              <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Phonenumber: {datas.phonenumber}</Card.Body>
              <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Gender: {datas.gender}</Card.Body>
              {/* <Button className='btn btn-outline-dark' size='sm' >Edit Profile</Button> */}
            </Card.Body>
          </Card>
          </center>
        </div>
      </div>


</div>
</div>
<br/>
      <h3 className='text-center' style={{fontFamily:"revert"}}>Assigned Employees</h3>
      <br />
      <div className='row'>
        <table className='table table-striped table-bordered'>
          <thead>
            <tr>
              <th>Manager EmailID</th>
              <th>Employee ID</th>
              <th>Employee FirstName</th>
              <th>Employee EmailId</th>
              <th>Employee Phonenumber</th>
            </tr>
          </thead>

          <tbody>
            {data.map((assign) => (
              <tr>
                <td>{assign.split(",")[0]}</td>
                <td>{assign.split(",")[1]}</td>
                <td>{assign.split(",")[2]}</td>
                <td>{assign.split(",")[3]}</td>
                <td>{assign.split(",")[4]}</td>
              </tr>

            ))}


          </tbody>

        </table>
      </div>

      
      </Container>
    </div>
    // </AutoLogout>
  )

}

export default ManagerHomepage;














