import axios from 'axios'
import React, { useEffect, useState } from 'react'
import { Button, Card, Container } from 'react-bootstrap'
import Sidebar from './SidebarEmployee'


const EmployeeHomePage = () => {

  const [data, setdata] = useState({})
 

  useEffect(() => {
    axios.get(`http://localhost:8085/Admin/gettoken/${window.localStorage.getItem('jwt')}`)
      .then(response => {
        axios.get(`http://localhost:8081/adminemployee/get/${response.data}`)
          .then(responses => {
            // setdata(JSON.parse(JSON.stringify(responses.data)))
            console.log(responses.data)
            setdata(responses.data)
            console.log(data)
          }, [])
        
      })
  }, []);

 
  return (

    <div>
      <Sidebar />

      <Container>
        <div className='catlog container'>

          <div className='row'>

                <div className='col-md-3 prod'>
                  <div className='catlogclass'>
                    <center>
                    <Card style={{ width: '25rem',marginTop:"10px",height:"500px",position:"relative",left: "350px" }} >
                      <center>
                    <Card.Img  variant="top" style={{height:"150px",width:"180px"}} src="data:image/png;base64," />
                    </center>
                      <Card.Body>
                        
                        <Card.Title>{data.lastname + ' ' +data.firstname}</Card.Title>
                        <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Email Address: {data.email}</Card.Body>
                        <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Phonenumber: {data.phonenumber}</Card.Body>
                        <Card.Body style={{fontWeight: "bold",marginLeft:"0px" ,float:"left"}}>Gender: {data.gender}</Card.Body>
                        <Card.Body style={{fontWeight: "bold" ,float:"left"}}>Assigned Manager: {data.manageremail}</Card.Body>
                        {/* <Button className='btn btn-outline-dark' size='sm' >Edit Profile</Button> */}
                      </Card.Body>
                    </Card>
                    </center>
                  </div>
                </div>


          </div>
        </div>
      </Container>
    
    </div>
  )
}

export default EmployeeHomePage
